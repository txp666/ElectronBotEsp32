...
void Screen::WriteFrameBuffer(uint8_t *_buffer, uint32_t _len, bool _isAppend)
{
    ...
    ChipSelect(true);
    _isAppend ? WriteCommand(0x3C) : // MEM_WR_CONT
        WriteCommand(0x2C);          // MEM_WR
    WriteData(_buffer, _len);
...
{
    digitalWrite(resetPin, _enable ? LOW : HIGH);
}

void Screen::SetDataOrCommand(bool _isData)
{
    digitalWrite(dcPin, _isData ? HIGH : LOW);
}
...
void Screen::WriteData(uint8_t *_data, uint32_t _len)
{
    ChipSelect(true);       // 使能屏幕
